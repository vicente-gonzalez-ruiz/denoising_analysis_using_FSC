...
\begin{enumerate}
\item Zero-mean Gaussian noise, where
  $\mathbf{N}_i\sim\mathcal{N}(\mu=0,\sigma^2)$, representing $\sigma$
  the noise level.
\item Zero-mean uniform noise, where
  $\mathbf{N}_i\sim\mathcal{U}(a=-c, b=c)$.
\end{enumerate}

Notice that in all these methods, it's hold that
\begin{equation}
