...
samples of ${\mathbf X}$, even if the random variable
$\{{\mathbf N}^{(i)}_j\}$ follow a i.i.d. distribution. Luckily, this
fact does not pose a major inconvenience to continue satisfying
Eq.~\ref{eq:noise_expectation}, (and therefore, also
Eq.~\ref{eq:averaging_result}) if the intensity variation due to noise
that each sample suffers between different tensor instances (the
...
\item Poisson noise ($\mathbf{N}_i\sim\mathcal{P}(\lambda)$), also called shot noise is defined by a PMD (Probability Mass Distribution)\footnote{Also
  ...
  \end{equation}
\item Rician noise ($\mathbf{N}_i\sim\mathcal{R}(\sigma)$), with PDF
\end{enumerate}
