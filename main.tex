...
White Gaussian noise: independent and identically distributed and can
be found as $N\~\mathcal{N}(\mu,\sigma^2)$ where $\sigma$ represents
the noise level.
...
distribution. 1D filtering is represented by
\begin{equation}
...
\begin{equation}
  \hat{\mathbf{Y}}(\sigma) = \Big(\big({\mathbf Y}*^{(\text{Z})}{\mathbf h}(\sigma)\big)*^{(\text{Y})}{\mathbf h}(\sigma)\Big)*^{(\text{X})}{\mathbf h}(\sigma),
    \label{eq:3DGD}
\end{equation}
where ${\mathbf s}*^{(d)}{\mathbf h}$ is the 1D convolution applied to the dimension $d$ of the signal ${\mathbf s}$ and the 1D filter ${\mathbf h}$.
...
$\sigma$ is not (objectively) optimizable using only pure-distortion
metrics.
